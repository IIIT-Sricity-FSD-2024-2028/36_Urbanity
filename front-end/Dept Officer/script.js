...
function renderValidateAssignComplaints() {
  ...
  const complaints = getOfficerScopedComplaints().filter((entry) => {
    const status = String(entry.status || "").toLowerCase();
    if (!actionableStatuses.has(status)) {
      return false;
    }

    const assignment = getOfficerAssignmentByComplaintId(entry.id);
...
function verifyResolution(id, approved) {
  ...
  if (assignment) {
    window.MockDataAPI.update("assignments", assignment.id, approved
      ? {
          status: "Completed",
          verifiedAt: new Date().toLocaleString(),
        }
      : {
          // Rejection sends the assignment back as a fresh pending task for the worker.
          status: "Pending",
          remarks: "",
          proofMedia: [],
          completedAt: null,
  ...
  window.MockDataAPI.update("complaints", complaintId, {
    ...
    verifiedAt: new Date().toLocaleString(),
    resolutionSubmittedAt: approved ? undefined : null,
    resolutionSummary: approved ? undefined : "",
    resolutionMedia: approved ? undefined : [],
    latestWorkerUpdate: approved ? undefined : "",
    lastUpdatedAt: approved ? undefined : null,
    workStartedAt: approved ? undefined : null,
  });

  refreshOfficerComplaintViews();
  showOfficerToast(
    `Complaint ${complaintId} has been ${approved ? "verified and resolved" : "sent back to pending"}.`,
    "success",
  );
}
