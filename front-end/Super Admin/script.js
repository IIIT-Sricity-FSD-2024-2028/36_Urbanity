(function () {
  ...
  const state = { user: null, communities: [], subscriptions: [], towers: [], floors: [], apartments: [], users: [], workers: [], complaints: [], report: null, dashboard: null, previewUrl: null };
  ...
  function renderCommunities() {
    document.getElementById("communities-content").innerHTML = table(["Community", "Address", "Description", "Community Admin", "Contracted capacity", "Payment", "Actions"], state.communities.map((community) => {
      const subscription = state.subscriptions.find((item) => item.communityId === community.id);
      const communityAdmin = state.users.find((user) => user.role === "COMMUNITY_ADMIN" && user.communityId === community.id);
      const details = subscription ? `${subscription.contractedTowers} towers · ${subscription.contractedApartments} apartments` : "";
      const payment = subscription ? `<span class="badge">${escape(subscription.status)}</span>` : "";
      return row([escape(community.name), escape(community.address), escape(community.description || ""), escape(communityAdmin ? `${communityAdmin.name} (${communityAdmin.email})` : ""), escape(details), payment, `<span class="actions"><button type="button" class="secondary" data-edit-community="${escape(community.id)}">Edit</button><button type="button" class="danger" data-delete-community="${escape(community.id)}">Delete</button></span>`]);
    }));
    const adminCommunity = document.getElementById("admin-community");
    if (adminCommunity) adminCommunity.innerHTML = `<option value="">Select a community</option>${state.communities.map((community) => `<option value="${escape(community.id)}">${escape(community.name)}</option>`).join("")}`;
  }
  function renderAdmins() {
    if (!document.getElementById("admins-content")) return;
  ...
  async function loadData() {
    const results = await Promise.all([api("/dashboard/summary"), api("/reports/overview"), api("/communities"), api("/subscriptions"), api("/towers"), api("/floors"), api("/apartments"), api("/users"), api("/workforce/workers"), api("/complaints")]);
    [state.dashboard, state.report, state.communities, state.subscriptions, state.towers, state.floors, state.apartments, state.users, state.workers, state.complaints] = results.map(data);
    renderAll();
    renderTopbar();
  }
  ...
  function openCommunityCreateForm() {
    openDetail("Create community", `<form id="community-create-form"><label>Name<input name="name" required maxlength="120"></label><label>Address<input name="address" required maxlength="250"></label><label>Description<textarea name="description" maxlength="500"></textarea></label><label>Community Admin name<input name="adminName" required maxlength="120"></label><label>Community Admin email<input name="adminEmail" type="email" required maxlength="160"></label><label>Temporary password<input name="adminPassword" type="password" required minlength="8" maxlength="128"></label><label>Contracted towers<input class="capacity-input" name="contractedTowers" type="number" min="1" step="1" required></label><label>Contracted apartments<input class="capacity-input" name="contractedApartments" type="number" min="1" step="1" required></label><div class="actions"><button id="cancel-community-create" type="button" class="secondary">Cancel</button><button data-create-community type="submit">Create community</button></div></form>`);
    const form = document.getElementById("community-create-form"); document.getElementById("cancel-community-create").onclick = closeDetail; form.onsubmit = (event) => { event.preventDefault(); createCommunity(form); };
  }
  async function createCommunity(panel) { const values = panelValues(panel); const button = panel.querySelector("[data-create-community]"); values.contractedTowers = Number(values.contractedTowers); values.contractedApartments = Number(values.contractedApartments); if (!values.name?.trim() || !values.address?.trim() || !values.adminName?.trim() || !values.adminEmail?.trim() || !values.adminPassword || !Number.isInteger(values.contractedTowers) || values.contractedTowers < 1 || !Number.isInteger(values.contractedApartments) || values.contractedApartments < 1) return notice("Complete the community, Community Admin, and positive contracted-capacity fields."); button.disabled = true; try { const created = data(await api("/communities", { method: "POST", body: values })); updateRecord("communities", created.community); updateRecord("users", created.admin); updateRecord("subscriptions", created.subscription); renderCommunities(); renderAdmins(); renderUsers(); closeDetail(); await refreshSummary(); notice("Community created with payment pending.", true); } catch (error) { notice(error.message || "Unable to create community."); } finally { button.disabled = false; } }
  ...
  function editCommunity(community) {
    openDetail("Edit community", `<form id="community-edit-form"><label>Name<input name="name" value="${escape(community.name)}" required maxlength="120"></label><label>Address<input name="address" value="${escape(community.address)}" required maxlength="250"></label><label>Description<textarea name="description" maxlength="500">${escape(community.description || "")}</textarea></label><div class="actions"><button type="button" class="secondary" id="cancel-community-edit">Cancel</button><button type="submit">Save changes</button></div></form>`);
    document.getElementById("cancel-community-edit").onclick = closeDetail;
    document.getElementById("community-edit-form").onsubmit = async (event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); try { const updated = data(await api(`/communities/${community.id}`, { method: "PATCH", body: { name: values.name.trim(), address: values.address.trim(), ...(values.description.trim() ? { description: values.description.trim() } : { description: "" }) } })); updateRecord("communities", updated); renderCommunities(); closeDetail(); notice("Community details updated.", true); } catch (error) { notice(error.message || "Unable to update community."); } };
  }
  async function communityAction(event) { const editId = event.target.dataset.editCommunity; const deleteId = event.target.dataset.deleteCommunity; if (!editId && !deleteId) return; event.preventDefault(); event.stopPropagation(); if (deleteId) { if (!window.confirm("Delete this community? This may affect associated towers, residents, and workers.")) return; try { await api(`/communities/${deleteId}`, { method: "DELETE" }); state.communities = state.communities.filter((item) => item.id !== deleteId); renderCommunities(); await refreshSummary(); notice("Community deleted.", true); } catch (error) { notice(error.message || "Unable to delete community."); } } if (editId) { const community = state.communities.find((item) => item.id === editId); if (community) editCommunity(community); } }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelector("[data-open-community-create]").addEventListener("click", openCommunityCreateForm);
